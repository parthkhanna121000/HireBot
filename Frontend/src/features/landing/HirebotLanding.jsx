...
// // ─── Navbar ───────────────────────────────────────────────────────────────────
// // Replace only the Navbar function in HirebotLanding.jsx with this version.
// // It reads the auth context and shows an "Admin Dashboard" link for admin users,
// // or the normal "Log in / Get started" buttons for guests.
...
//         <span className="logo-main">Hire<span className="logo-bold">Bot</span><span className="logo-ai">AI</span></span>
//       </div>
...
//       { text: "Limited interview report / day",  ok: true },
//       { text: "Job listings & apply",      ok: true },
...
//       { text: "Unlimited interview prep",    ok: true },
//       { text: "All Free features",           ok: true },
//       { text: "Priority AI processing",      ok: true },
//       { text: "Advanced hiring insights",    ok: true },
//       { text: "PDF exports & AI rewriter",   ok: true },
//     ],
//     cta: "Upgrade to Pro",
//   },
//   {
...
//       { text: "12 months access",            ok: true },
//       { text: "~₹333/month effective",       ok: true },
//       { text: "Priority AI processing",      ok: true },
//       { text: "Advanced hiring insights",    ok: true },
//       { text: "PDF exports & AI rewriter",   ok: true },
//     ],
...
//             ctx.strokeStyle = `rgba(99,102,241,${0.05 * (1 - dist / 130)})`; ctx.lineWidth = 0.5; ctx.stroke();
//           }
//         }
//       }
...
//             animate={{ strokeDashoffset: circ * (1 - 0.78) }} transition={{ duration: 1.6, ease: "easeOut" }}/>
//         </svg>
...
//             <div className="ats-check-icon" style={{ background:ok?"rgba(16,185,129,0.2)":"rgba(244,63,94,0.2)", color:ok?"#10b981":"#f43f5e" }}>
...
//           initial={{ opacity:0, x:-12 }} animate={{ opacity:1, x:0 }} transition={{ delay:i * 0.1, duration:0.4 }}>
...
//         <div className="nav-divider" />
//         <button className="btn-login" onClick={() => navigate("/login")}>Log in</button>
//         <button className="btn-signup" onClick={() => navigate("/register")}>Get started free <Icon.ArrowRight /></button>
//       </div>
//       <button className="nav-hamburger"><Icon.Menu /></button>
//     </nav>
//   );
...
//         <motion.div className="hero-eyebrow" initial={{ opacity:0, y:-16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.55 }}>
//           <div className="badge-dot" />AI-powered resume intelligence<Icon.Sparkles />
//         </motion.div>
//         <motion.h1 initial={{ opacity:0, y:28 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.65, delay:0.1 }}>
//           Get Hired Faster<br /><span className="highlight">with AI Intelligence</span>
//         </motion.h1>
//         <motion.p className="hero-sub" initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:0.6, delay:0.22 }}>
//           Upload your resume, get an instant AI score, compare against any job description, and receive precise suggestions to land more interviews.
//         </motion.p>
//         <motion.div className="hero-ctas" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.55, delay:0.32 }}>
//           <button className="btn-primary-lg" onClick={() => navigate("/resume-analyzer")}><Icon.Upload /> Upload Resume</button>
//           <button className="btn-secondary-lg" onClick={() => navigate("/recruiter")}>Post a Job →</button>
//         </motion.div>
//         <motion.div className="hero-stats" initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:0.55, delay:0.45 }}>
//           {[
...
//           ))}
//         </motion.div>
//         <motion.div className="score-widget-demo" initial={{ opacity:0, y:40, scale:0.96 }} animate={{ opacity:1, y:0, scale:1 }} transition={{ duration:0.7, delay:0.6 }}>
...
//               <div className="sw-bar-top"><span className="sw-bar-label">{b.label}</span><span className="sw-bar-pct">{b.pct}%</span></div>
//               <div className="sw-bar-track">
//                 <motion.div className="sw-bar-fill" style={{ background:b.color }} initial={{ width:0 }} animate={{ width:`${b.pct}%` }} transition={{ duration:1.3, ease:"easeOut", delay:0.9 }}/>
//               </div>
//             </div>
//           ))}
//           <div className="sw-tags">
//             <div className="sw-tag-label">Missing skills</div>
//             <div className="sw-tag-list">{["Docker","Kubernetes","CI/CD"].map((s) => <span key={s} className="tag-missing">{s}</span>)}</div>
//           </div>
//         </motion.div>
//       </div>
//     </section>
...
//   const cards = [
//     { cls:"seeker", icon:"🎯", heading:"For Job Seekers", dotColor:"#f43f5e", items:["Resumes don't highlight key skills or experience effectively","ATS rejects most applications before a human sees them","Hard to know which skills are missing for a target role","Interview preparation is difficult without personalized guidance","Job applications go unnoticed by recruiters"] },
//     { cls:"recruiter", icon:"🏢", heading:"For Recruiters & HR Teams", dotColor:"#f59e0b", items:["Screening hundreds of resumes manually is time-consuming","Hard to quickly identify the top-fit candidates","Shortlisting has bias and human error","Lack of AI insights for improving job postings","Top talent often gets missed in the volume"] },
//   ];
//   return (
//     <section ref={ref}>
...
//               initial={{ opacity:0, y:32 }} animate={inView ? { opacity:1, y:0 } : {}} transition={{ duration:0.55, delay:i * 0.15 }}>
...
//               initial={{ opacity:0, y:40 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.55, delay:i * 0.15 }}>
...
//               onClick={() => setActive(f.id)} whileTap={{ scale:0.94 }}>{f.label}
//             </motion.button>
//           ))}
//         </div>
...
//                 <span className="fs-badge-dot" style={{ background:feat.badgeColor }} />{feat.label}
//               </div>
...
//           <p className="section-sub">No subscriptions, no auto-renewals. Pay once for Pro access and unlock everything.</p>
//         </motion.div>
...
//                     transition={{ delay: 0.3 + i * 0.1 + fi * 0.04 }}
//                   >
//                     <div className="lp-feature-icon" style={{ background: f.ok ? `${plan.color}22` : "rgba(255,255,255,0.04)", color: f.ok ? plan.color : "#333" }}>
...
//                 style={plan.highlight ? { "--cta-color": plan.color } : {}}
...
//                   <Icon.Shield /> Razorpay secured · PCI DSS compliant
//                 </p>
//               )}
//             </motion.div>
//           ))}
...
//           <span>Want to compare all features in detail?</span>
//           <button className="pricing-preview__link" onClick={() => navigate("/pricing")}>
//             View full pricing page <Icon.ArrowRight />
//           </button>
//         </motion.div>
//       </div>
//     </section>
...
//               initial={{ opacity:0, x:-24 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }} transition={{ duration:0.45, delay:i * 0.08 }}>
//               <div className="j-num" style={{ borderColor:s.color, color:s.color, background:`${s.color}14` }}>{i + 1}</div>
...
//         <motion.div className="rs-card" initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.5 }}>
//           <div>
//             <div className="section-label" style={{ marginBottom:8 }}>For Recruiters</div>
//             <h3 style={{ fontFamily:"var(--font-display)", fontSize:"1.5rem", fontWeight:900, color:"var(--c-text1)", marginBottom:10, letterSpacing:"-0.3px" }}>Stop Drowning in Resumes</h3>
//             <p style={{ fontSize:14, color:"var(--c-text2)", lineHeight:1.8, maxWidth:500 }}>Post a job and let AI instantly rank every applicant with a match score, skill summary, and red flags — so you spend time only on candidates who actually fit.</p>
//             <div className="rs-tags">
//               {["AI Candidate Ranking","Match Score","Bias Reduction","JD Optimization"].map((t) => <span key={t} className="rs-tag">{t}</span>)}
//             </div>
//           </div>
//           <button className="rs-btn" onClick={() => navigate("/recruiter")}>Post a Job <Icon.ArrowRight /></button>
//         </motion.div>
//       </div>
//     </section>
//   );
...
//       <motion.div className="cta-inner" initial={{ opacity:0, scale:0.96 }} whileInView={{ opacity:1, scale:1 }} viewport={{ once:true }} transition={{ duration:0.55 }}>
...
//           <button className="btn-white" onClick={() => navigate("/resume-analyzer")}>Upload Your Resume — Free</button>
//           <button className="btn-outline-white" onClick={() => navigate("/demo")}>View Demo</button>
//         </div>
//       </motion.div>
//     </div>
//   );
...
//         {["Privacy","Terms","Blog","Pricing","Contact"].map((l) => <span key={l}>{l}</span>)}
//       </div>
//       <div style={{ fontSize:12, color:"var(--c-text3)" }}>© 2025 HireBot AI. All rights reserved.</div>
//     </footer>
//   );
// }
  ...
  return (
    <nav className="landing-nav" style={{ boxShadow: scrolled ? "0 4px 30px rgba(0,0,0,0.5)" : "none" }}>
      ...
      <div className="nav-links">
        ...
        {user && !isAdmin && (
          <>
            <button className="btn-login" onClick={() => navigate(user.role === "recruiter" ? "/recruiter" : "/dashboard")}>
              Dashboard
            </button>
            <button className="btn-login" onClick={onLogout}>Log out</button>
          </>
